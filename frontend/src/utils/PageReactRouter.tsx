...
export function Link({ to, children, className, active, disabled }: { to: string, children: any, className?: string, active?: boolean, disabled?: boolean }) {
  ...
  return <div onClick={handleClick} className={(className || '') + (isActive ? ' active' : '') + ' link-cursor'} style={{ cursor: 'pointer' }}>{children}</div>
}
