...
/**
 * Types of moves that can be recorded during gameplay
 */
export type MoveType = 'fill' | 'unfill' | 'mark-empty' | 'unmark-empty';

/**
 * A single move recorded during gameplay
 * Records what action was taken, where, and when
 */
export interface PlayerMove {
  a: MoveType; // Type of action performed // a for action
  r: number; // Row index of the cell
  c: number; // Column index of the cell
  t: number; // Timestamp when the move was made (milliseconds since epoch)
}
...
export interface GameState {
  ...
  progress: number;
  timestamp: number;
  completedRows?: number[]; // Array of row indices that are completed
  completedCols?: number[]; // Array of column indices that are completed
  moves?: PlayerMove[]; // Sequence of moves made during this game session
  sessionHash?: string; // Hash generated by server for this game session
  sessionTimestamp?: number; // Timestamp from server when session was started
}
...
export function saveGameState(state: GameState): void {
  try {
    // Directly serialize arrays without conversion
...
export function loadGameState(): GameState | null {
  try {
    ...
    // Directly use arrays without conversion
 ...
 * Returns arrays of row and column indices that are complete
 */
export function computeCompletedRowsAndCols(
  playerGrid: boolean[][],
  solutionGrid: boolean[][]
): { completedRows: number[]; completedCols: number[] } {
...
export function isGameWon(gameState: GameState): boolean {
  ...
  const allRowsCompleted = completedRows.length === gridHeight;
  const allColsCompleted = completedCols.length === gridWidth;

  return allRowsCompleted && allColsCompleted;
}

/**
 * Record a player move with the current timestamp
 * Helper function to create a PlayerMove object
 */
export function recordMove(type: MoveType, row: number, col: number): PlayerMove {
  return {
    a: type,
    r: row,
    c: col,
